...
import { assert, request, runId, token } from './e2e-support.mjs';
...
async function status(path, accessToken) {
...
const room = await request(proxy, userToken, 'POST', '/v1/rooms', { title: `Metrics route ${runId}` });
await request(proxy, userToken, 'GET', `/v1/rooms/${room.id}`);
...
const metricSamples = metrics.text.split(/\r?\n/).filter((line) => (
...
for (const sensitiveValue of [room.id, userId, runId]) {
  assert(!metrics.text.includes(sensitiveValue), `Metrics leaked high-cardinality value ${sensitiveValue}`);
}
